FIS <- function(layers) {
  ...
  f_gradient <- function(f, over_f, under_f, fmax) {
  ...
  rescale_fprime_crit <- function(fish_stat_df,
    ...
    underfishing_th <- 0; overfishing_th  <- 1.2
  ...
  write.csv(stock_score_df, "~/github/bc-fisheries/scenarios/no_underfishing_penalty/output/stock_scores.csv")
  ...
  #save for plotting
  write_csv(fis_status, '~/github/bc-fisheries/scenarios/no_underfishing_penalty/output/fis_status.csv')
